...
from kitti_player.msg import BboxL, BboxLes
...
def get_bev(scan, label_list, bboxes):
    ...
    truth_color = (0,128,0)
    for bbox in bboxes.bboxl:
        print([int(bbox.minx*10), int(bbox.miny*10)],[int(bbox.maxx*10),int(bbox.maxy*10)])
        cv2.rectangle(intensity,[int(bbox.minx*10), int(400-bbox.miny*10)],[int(bbox.maxx*10),int(400-bbox.maxy*10)],truth_color,thickness=2)
...
class KittiSub(object):
    def __init__(self, name, device):
        self.ready = False
        self.sub = rospy.Subscriber('/kitti_player/ground_truth', BboxLes, self.boxcb, queue_size=1)
    ...
    def cb(self, data):
        if self.ready:
            ...
            # print("preprocess elapsed in {} seconds".format(t_preprocess - t))
            ...
            with torch.no_grad():
                ...
                # print("Input Formatting elapsed in {} seconds".format(t_in_format-t_preprocess)) 
                
                predictions = self.net(input)

                t_net = time.time()
                # print(t_net-t_in_format)
                dt = t_net-t_in_format
            ...
            t_net = time.time()
            # print("Network elapsed in {} seconds".format(t_net-t_in_format-dt)) 
            
            intensity = get_bev(input_np, preds, self.bboxes)
            publishImage(intensity, self.pub)
            t_cb = time.time()
            # print("Callback ended in {} seconds".format(t_cb-t_net))
            # print("Total runtime {}".format(t_cb-t))
