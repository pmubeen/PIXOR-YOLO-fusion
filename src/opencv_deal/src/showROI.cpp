...
public:
	...
	void ROIcb(const sensor_msgs::PointCloud2 &input)
	{
		...
		std::string calibFilePath = "src/opencv_deal/src/calib.txt";
		...
		for (int i = 0; i < Rec.points.size(); ++i)
		{
			...
			if (!lastRec.empty())
			{
				for (int k = 0; k < lastRec.points.size(); ++k)
				{
					...
					if (ifmaxx && ifmaxy && ifminx && ifminy)
					{
						flag = 0;
						break;
					}
	...
	bool readOdometryCalib(const std::string &calibFile, cv::Mat &projectionMatrix, cv::Mat &cameraMatrix, cv::Mat &velo2camera)
	{
		...
		while (!inputFile.eof())
		{
			...
			if (tag == "P2:")
			{
				inputString >> cameraMatrix.at<float>(0, 0) >> cameraMatrix.at<float>(0, 1) >> cameraMatrix.at<float>(0, 2) >> cameraMatrix.at<float>(0, 3) >> cameraMatrix.at<float>(1, 0) >> cameraMatrix.at<float>(1, 1) >> cameraMatrix.at<float>(1, 2) >> cameraMatrix.at<float>(1, 3) >> cameraMatrix.at<float>(2, 0) >> cameraMatrix.at<float>(2, 1) >> cameraMatrix.at<float>(2, 2) >> cameraMatrix.at<float>(2, 3);
			}

			if (tag == "Tr:")
			{
				inputString >> velo2camera.at<float>(0, 0) >> velo2camera.at<float>(0, 1) >> velo2camera.at<float>(0, 2) >> velo2camera.at<float>(0, 3) >> velo2camera.at<float>(1, 0) >> velo2camera.at<float>(1, 1) >> velo2camera.at<float>(1, 2) >> velo2camera.at<float>(1, 3) >> velo2camera.at<float>(2, 0) >> velo2camera.at<float>(2, 1) >> velo2camera.at<float>(2, 2) >> velo2camera.at<float>(2, 3);
			}
		}
		inputFile.close();
