	...
	void processBEV(const void *nparray, unsigned char *data, uint32_t height, uint32_t width, uint32_t point_step, uint32_t row_step)
	{
		...
		for (size_t i = 0; i < height; i++)
		{
			...
			for (size_t j = 0; j < width; j++)
			{
				...
				u4 = (data[offset + 16] << 0) | (data[offset + 17] << 8) | (data[offset + 18] << 16) | (data[offset + 19] << 24);

				//printf("input %i: %d\n", offset/(4*point_step), u4);
				...
				if (f1 > X_MIN && f2 > Y_MIN && f3 > Z_MIN && f1 < X_MAX && f2 < Y_MAX && f3 < Z_MAX)
				{
					...
					int y = getY(f2);
					int z = getZ(f3);
					// if course ground removal is needed
					// if (z >= 15)
					// {
					// 	*(data_cube + at3(y, x, z)) = 1;
					// 	*(data_cube + at3(y, x, 35)) += f4;
					// }
